...
%% Filter out uniformly expressed features
...
%         % ****************************************
        ...
        for i = 1:max_col     
            ...
            Delta_mean(:, i) = Class_mean - Null_mean;
            SE(:, i) = arrayfun(@(x, y) sqrt(x + y), Class_var_weighted, Null_var_weighted);
            T_stat(:, i) = Delta_mean(:, i) ./ SE(:, i);
            DoF(:, i) = arrayfun(@(x, n1, y, n2) (x + y)^2 / ( (x^2 / (n1-1)) + (y^2 / (n2-1)) ), Class_var_weighted, Class_N, Null_var_weighted, Null_N); 
            feature_Pvals(1:n, i) = arrayfun(@(ratio, dfe) tcdf(-ratio, dfe), T_stat(:, i), DoF(:, i));
            logFold(1:n, i) = log2(2.^Class_mean ./ 2.^Null_mean);
        end
        feature_Pvals = min(1, feature_Pvals .* (n*max_col)); % Bonferroni correction
        significant_features_mask = min(feature_Pvals, [], 2) <= params.pval_threshold;
        ...
        feature_Pvals = feature_Pvals(significant_features_mask, :);
        n = n - numel(debug.ttest_excluded); 
%% Pick top-ranked features according to log ratio of expression values        
        [~, best_cut] = min(feature_Pvals, [], 2);
        
        feature_score = logFold(sub2ind(size(logFold), (1:n)', best_cut));
