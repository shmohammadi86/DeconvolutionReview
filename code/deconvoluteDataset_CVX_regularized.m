function [estimatedC, trace] = deconvoluteDataset_CVX_regularized(Y, X, varargin)
    n = size(Y, 1);
    p = size(Y, 2);
    q = size(X, 2);
            ...
            for sample_id = 1:p
                ...
                cvx_begin quiet
                    ...
                    switch params.regularize
                        case 'none'
                            minimize( norm(X*w- y, 2) )
                        case 'L1'
                            minimize( norm(X*w- y, 2) + params.lambda1*norm(w, 1)) 
                        case 'L2'
                            minimize( norm(X*w- y, 2) + params.lambda2*norm(w, 2))                                                        
                    end
                    subject to
                        if(params.NN)
                            0 <= w;
