function [estimatedC, trace] = deconvoluteDataset_CVX_weighted_regularized(Y, X, W, varargin)
    n = size(Y, 1);
    p = size(Y, 2);
    q = size(X, 2);
    ...
    args.addParamValue('regularize', 'none', @(x) ischar(x)); %none, L2, L1, elastic
    args.addParamValue('alpha', 0, @(x) isscalar(x) & x >= 0); % relaxation for adaptive range filtering
    args.addParamValue('filter_type', 'none', @(x) ischar(x)); 
                ...
                if(nnz(~bad_feature_mask) ==  0)
                    ...
                    cvx_begin quiet
                        ...
                        switch params.regularize
                            case 'none'
                                minimize( norm(W*(X*w- y), 2) )
                            case 'L1'
                                minimize( norm(W*(X*w- y), 2) + params.lambda1*norm(w, 1)) 
                            case 'L2'
                                minimize( norm(W*(X*w- y), 2) + params.lambda2*norm(w, 2))                                                        
                        end
                        subject to
                            if(params.NN)
                                0 <= w;
         ...
         case 'L1'
            ...
            for sample_id = 1:p
                ...
                cvx_begin quiet
                    ...
                    switch params.regularize
                        case 'none'
                            minimize( norm(X*w- y, 1) )
                        case 'L1'
                            minimize( norm(X*w- y, 1) + params.lambda1*norm(w, 1)) 
                        case 'L2'
                            minimize( norm(X*w- y, 1) + params.lambda2*norm(w, 2))                                                        
                    end
                    subject to
                        if(params.NN)
                            0 <= w;
